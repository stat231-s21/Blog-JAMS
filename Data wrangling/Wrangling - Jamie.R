...
school_water_data_by_country <- read_csv("data/basic water access in schools by country.csv") %>%
...
primary_enrollment_rates <- read_csv("data/gross-enrollment-ratio-in-primary-education.csv") %>%
  group_by(Entity) %>%
...
secondary_enrollment_rates <- read_csv("data/gross-enrollment-ratio-in-secondary-education.csv") %>%
  group_by(Entity) %>%
  slice(which.max(Year)) %>%
  select(Entity, GrossEnrolmentRatio) %>%
...
# Writing csv file
write_csv(final_scatter_data, "data/school-enrollment-and-water-access.csv")
